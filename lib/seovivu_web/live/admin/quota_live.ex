defmodule SeovivuWeb.Admin.QuotaLive do
  @moduledoc "Admin page: CRUD for main-toolkit service packages (credits + days)."
  ...
  def mount(_params, _session, socket) do
    {:ok,
     socket
     |> assign(:page_title, "Thiết lập gói")
  ...
  def render(assigns) do
    ...
    <Layouts.admin
      ...
      active={:quota}
      page_title="Thiết lập gói"
      flash={@flash}
    >
      <div class="mb-6 flex flex-wrap items-end justify-between gap-4">
        <div>
          <h2 class="text-3xl font-bold tracking-tight text-ink">Thiết lập gói</h2>
          <p class="mt-1 text-ink-muted">Các gói dịch vụ admin có thể cấp cho người dùng.</p>
        </div>
      ...
      <.table :if={@packages != []} id="packages" rows={@packages}>
      ...
      >
        <div class="flex items-start justify-between">
          <div>
            ...
            <p class="text-sm text-ink-muted">Gói dịch vụ chính (credit + số ngày).</p>
          </div>
          <button
            type="button"
            phx-click="close"
        ...
        <.form for={@form} phx-submit="save" class="mt-5 space-y-1">
          <.input field={@form[:name]} label="Tên" placeholder="Gói cơ bản" required />
